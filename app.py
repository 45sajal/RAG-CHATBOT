...
# # Search documents (retrieve top_k documents)
...
# # Generate similar queries using the LLM
# def generate_similar_queries(query, num_queries=3):
#     input_text = f"Generate similar queries to: {query}"
#     inputs = tokenizer(
...
# # Rank documents using reciprocal ranking
# def rank_documents(retriever, similar_queries, top_k=3):
#     doc_scores = {}

#     for sim_query in similar_queries:
#         results = search_documents(sim_query, retriever, top_k)

#         for rank, result in enumerate(results, start=1):
#             # Calculate reciprocal rank score: 1 / (1 + rank)
#             rank_reciprocal_score = 1 / (
#                 1 + rank
#             )  # You can adjust the k value if needed

#             # If the document is already in doc_scores, add the score
#             if result.page_content not in doc_scores:
#                 doc_scores[result.page_content] = rank_reciprocal_score
#             else:
#                 doc_scores[result.page_content] += rank_reciprocal_score

#     # Sort documents based on the cumulative reciprocal rank score
...
# # Generate answer from conversational chain
...
#         input_text = (
#             f"The user is asking for a summary of prior responses, provide it in a concise in 250 words"
...
#     results = rank_documents(user_question, retriever, similar_queries)
